...
async function seedData(count: number) {
  ...
  try {
    for (i = 0; i < count; i++) {
      const characterName: string = faker.unique(faker.word.adjective, [8]);
      const color = avatarColor();
      const avatar = generateAvatar(characterName.charAt(0).toUpperCase(), color);
      ...
      console.log(`***SEEDING DATABASE*** - ${i + 1} / ${count} - ${characterName}`);
